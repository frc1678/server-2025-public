...
log = logging.getLogger("generate_test_data.py")
    ...
    def generate_for_each_datapoint_collection(self) -> dict:
        ...
        for datapoint_collection_name, datapoint_collection_values in datapoint_collections.items():
            if datapoint_collection_name[:2] == "--":
                continue
            if not isinstance(datapoint_collection_values, dict):
                continue
...
def ask_number() -> int:
    number = utils.input("What number of schema data structure do you want? ")
    return int(number)
